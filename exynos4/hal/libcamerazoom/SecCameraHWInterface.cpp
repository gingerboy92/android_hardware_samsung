...
status_t CameraHardwareSec::previewFimcThread()
{
    ...
    if (mPreviewRunning == false) {
        ...
        if (mRunningThread > 0) {
            mRunningThread--;
            LOGE(" mRunningthread-- : %d", mRunningThread);
        }
...
#ifdef USE_FACE_DETECTION
    if (mRunningSetParam) {
        LOGD("%s: skip preview fimc thread", __func__);
        return true;
    }
    mFDcallbackLock.lock();
    if (mUseInternalISP && (mMsgEnabled & CAMERA_MSG_PREVIEW_METADATA) && mPreviewRunning && !mRecordRunning && ((mFDcount % 20) == 0)) {
        mDataCb(CAMERA_MSG_PREVIEW_METADATA, mFaceDataHeap, 0, &mFaceData, mCallbackCookie);
    }
    mFDcallbackLock.unlock();
...
status_t CameraHardwareSec::recordFimcThread()
{
    ...
    if (mRecordRunning == true) {
...
#ifdef BOARD_USE_V4L2_ION
        numArray = mCurrentIndex;
#else
        recordingIndex = mCurrentIndex;
        mSecCamera->runRecordFimcOneshot(mCurrentIndex, (unsigned int)(mSecCamera->getShareBufferAddr(mCurrentIndex)));
        mSecCamera->getRecordPhysAddr(mCurrentIndex, &recordAddr);

        LOGV("index (%d), record PhyY(0x%08x) phyC(0x%08x) ", mCurrentIndex, recordAddr.phys.extP[0], recordAddr.phys.extP[1]);
        ...
        addrs[mCurrentIndex].type   = kMetadataBufferTypeCameraSource;
        addrs[mCurrentIndex].addr_y = recordAddr.phys.extP[0];
        addrs[mCurrentIndex].addr_cbcr = recordAddr.phys.extP[1];
        addrs[mCurrentIndex].buf_index = mCurrentIndex;
#endif

        // Notify the client of a new frame.
        if (mMsgEnabled & CAMERA_MSG_VIDEO_FRAME) {
            mDataCbTimestamp(timestamp, CAMERA_MSG_VIDEO_FRAME,
                             mRecordHeap[0], mCurrentIndex, mCallbackCookie);
        } else {
            mSecCamera->releaseRecordFrame(mCurrentIndex);
        }
    } else {
        mFimcLock.lock();
        if (mRunningThread > 0) {
...
status_t CameraHardwareSec::snapshotFimcThread()
{
    ...
    if (mCurrentIndex < 0) {
        LOGV("%s: doing nothing mCurrnet index < 0", __func__);
        return true;
    }
    ...
    if (mCapBuffer.virt.extP[0] == NULL)
        mSecCamera->getSnapshotAddr(&mCapBuffer);
