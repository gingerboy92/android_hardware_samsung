...
/* HDMI HPD Device details */

#define HPD_DEV         "/dev/HPD"
#define HPD_GET_I2C_CHANNEL _IOR('H',101,char *)

/**
 * @brief DDC device name.
 * Probe device info from kernel
 */
...
int DDCOpen()
{
    ...
    if (ioctl(fp_hpd, HPD_GET_I2C_CHANNEL, &channel)<0){
         LOGE("%s: : %s Failiure in probing i2c Details from kernel",__func__,channel);  
         return -1; 
    }

#if DDC_DEBUG
        LOGE("%s:close I2C_DDC channel: uses device %s",__func__,channel);
#endif 
 
    if ((ddc_fd = open(channel,O_RDWR)) < 0) {
        LOGE("%s: Cannot open I2C_DDC : %s",__func__,channel);
        ret = 0;
    }
...
int DDCClose()
{
    ...
    if (close(ddc_fd) < 0) {
#if DDC_DEBUG
        LOGE("%s: Cannot close I2C_DDC : %s",__func__,channel);
#endif
        ret = 0;
    }
